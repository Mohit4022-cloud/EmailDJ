...
If the current Vercel preview is protected or otherwise blocked and you only need to refresh the evidence snapshot, use `make launch-probe-web-app-readout`; it writes the same probe artifacts but exits 0 without clearing any launch blocker. Keep `make launch-probe-web-app` as the strict launch gate.
The checked-in `hub-api/reports/launch/*` files are point-in-time evidence snapshots. After every target commit deploy, Vercel deployment change, or deployed web-app input change, rerun `make launch-probe-web-app && make launch-audit` in the operator session before treating those reports as current launch proof. Report-only snapshot commits may advance git SHA without invalidating the deployed web-app probe.
