...
- Generated at: `2026-05-07T21:40:38.343322Z`
...
| `deployed_http_smoke` | `blocked` | `required_http_smoke_routes=['generate', 'remix']`<br>`route_gates={'generate': True, 'remix': True, 'preview': False}`<br>`localhost_smoke_provider_source_counts={'provider_stub': 60}`<br>`web_app_client_bundle_usable=False`<br>`web_app_probe_failures=['http_error:401', 'no_same_origin_bundle_assets_found', 'vite_hub_url_not_found_in_bundle', 'vite_preview_pipeline_not_found_in_bundle']` | `http_smoke_external_provider_missing_for_launch_mode:limited_rollout`<br>`web_app_deployment_probe_not_usable`<br>`web_app_deployment_probe:http_error:401`<br>`web_app_deployment_probe:no_same_origin_bundle_assets_found`<br>`web_app_deployment_probe:vite_hub_url_not_found_in_bundle`<br>`web_app_deployment_probe:vite_preview_pipeline_not_found_in_bundle` |
...
| 6 | Prove web app generate/remix/preset preview against deployed hub-api. | `blocked` | `deployed_http_smoke` | `http_smoke_external_provider_missing_for_launch_mode:limited_rollout`<br>`web_app_deployment_probe_not_usable`<br>`web_app_deployment_probe:http_error:401`<br>`web_app_deployment_probe:no_same_origin_bundle_assets_found`<br>`web_app_deployment_probe:vite_hub_url_not_found_in_bundle`<br>`web_app_deployment_probe:vite_preview_pipeline_not_found_in_bundle` | Limited rollout proves generate/remix by default; preview smoke is required only when preview is intentionally enabled. |
