...
def build_messages(
    *,
    ...
    system = (
        ...
        "RULE 5A - PRESERVE LIST IS BINDING.\\n"
        "Every sentence index provided in rewrite_context.preserve_sentence_indexes must remain verbatim unless a higher-priority issue explicitly targets it.\\n\\n"
    ...
    user = (
        ...
        "2) Output preserve_sentence_indexes for every untouched sentence index that must remain verbatim. Copy them from rewrite_context unless you have an explicit issue-backed reason not to.\\n"
        ...
        "10) If only one sentence is bad, do not rewrite the whole body; patch that sentence and keep the rest.\\n"
        "11) Validate the patch against schema, atoms grounding, preserve list discipline, CTA lock, preset contract, and budget plan.\\n\\n"
