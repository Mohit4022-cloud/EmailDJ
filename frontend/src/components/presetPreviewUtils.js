...
export function buildPresetPreviewPayload(context, preset) {
  const normalized = normalizePreviewContext(context || {});
  const effective = resolveEffectiveSliderState(normalized.global_slider_state, preset || {});
  const toAxis = (value) => clamp((Number(value) - 50) / 50, -1, 1);
  const offerLock = compactWhitespace(
    normalized.offer_lock || normalized.company_context.current_product || normalized.company_context.company_name || 'Current offering'
  );

  return {
    session_id: normalizeText(context?.session_id) || null,
    preset_id: String(preset?.id ?? ''),
    prospect: {
      name: normalized.prospect.name || 'there',
      title: normalized.prospect.title || 'Revenue Leader',
    ...
    prospect_first_name: normalized.prospect.first_name || null,
    research_text: ensureResearchText(normalized),
    offer_lock: offerLock,
    cta_offer_lock: compactWhitespace(normalized.company_context.cta_lock_text || normalized.company_context.cta_offer_lock) || null,
    cta_type: compactWhitespace(normalized.company_context.cta_type) || null,
    style_profile: {
      formality: toAxis(effective.formality),
      orientation: toAxis(effective.orientation),
      length: toAxis(effective.length),
      assertiveness: toAxis(effective.assertiveness),
    },
    company_context: {
