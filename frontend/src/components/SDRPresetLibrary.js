...
export class SDRPresetLibrary {
  ...
  markPreviewUnavailable(presets, context, message) {
    for (const preset of presets) {
      ...
      this.previewEntries.set(String(preset.id), {
        ...existing,
        status: 'error',
        errorMessage: String(message || 'Preview generation unavailable.'),
      });
    }
    this.renderPresetList();
    this.renderPreview(this.getPreviewPreset());
  ...
  async generateMissingPreviewPool(presets, context, contextHash) {
    if (!this.generatePreview) return;
    const selectedId = String(this.previewPresetId || '');
    ...
    const worker = async (preset) => {
      const inflightKey = buildPreviewCacheKey(contextHash, preset.id);
      if (this.inflightPreviews.has(inflightKey)) {
        await this.inflightPreviews.get(inflightKey);
        return;
      }
      const promise = this.fetchSinglePreview(preset, context, contextHash);
      this.inflightPreviews.set(inflightKey, promise);
      try {
        await promise;
      } finally {
    ...
    const concurrency = Math.max(1, this.maxConcurrentPreviews);
    const queue = [...ordered];
  ...
  async fetchSinglePreview(preset, context, contextHash) {
    const base = this.buildBasePreviewEntry(preset, context);
    const response = await this.generatePreview(buildPresetPreviewPayload(context, preset));
    ...
    const preview = {
      ...
      vibeTags:
        Array.isArray(response?.vibeTags) && response.vibeTags.length > 0
          ? response.vibeTags.map((tag) => String(tag)).slice(0, 4)
          : base.vibeTags,
      whyItWorks:
        Array.isArray(response?.whyItWorks) && response.whyItWorks.length > 0
          ? response.whyItWorks.map((item) => String(item)).slice(0, 3)
          : base.whyItWorks,
  ...
  setPreviewPreset(id) {
    ...
    if (this.previewFetchTimer) return;
    const base = this.buildBasePreviewEntry(preset, context);
